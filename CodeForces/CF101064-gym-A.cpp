// IDEA: write mathematical eqn for the condition which turns out to be a straight line. then take their point of intersection for their solution
...
Point pt[12];
int getpoints(double a,double b,double c,int pos){
	if(a!=0){
		pt[2*pos]=Point(c/a,0);
		pt[2*pos+1]=Point((c-b)/a,1);
	}
	else{
		pt[2*pos] = Point(0,c/b);
		pt[2*pos+1] = Point(1,c/b);
	}
}
double x[12],y[12];
int main(){
    ...
    getpoints(y[2]-y[1],y[3]-y[2]-h,-1*y[1],0);
    getpoints(x[2]-x[1]-w,x[3]-x[2],-1*x[1],1);
    if(LinesCollinear(pt[0],pt[1],pt[2],pt[3])){
    	//cout<<"das"<<endl;
    }
    else{
    	pt[0] = ComputeLineIntersection(pt[0],pt[1],pt[2],pt[3]);
    	cout<<setprecision(30)<<pt[0].x*w<<" "<<pt[0].y*h<<endl;
    }
    return 0;  
